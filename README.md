...
- **用户认证系统**：集成 Appwrite 云端认证服务，支持用户注册、登录和会话管理，确保数据安全和用户隐私。
...
OzonAssist 现在提供完整的用户认证功能：

### 首次使用

1. 启动应用后，将自动跳转到登录页面
2. 如果没有账户，点击"立即注册"创建新账户
3. 填写用户名、邮箱和密码（至少8位）
4. 同意服务条款后，点击"注册"完成账户创建
...
在应用顶部导航栏右侧，点击"登出"按钮即可安全退出当前账户。

### 配置说明

应用使用 Appwrite 作为认证服务，相关配置在 `.env` 文件中：

```env
VITE_APPWRITE_PROJECT_ID=你的项目ID
VITE_APPWRITE_PROJECT_NAME=项目名称
VITE_APPWRITE_ENDPOINT=Appwrite服务器地址
```
