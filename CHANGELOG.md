...
- **自动更新系统**: 集成 electron-updater，支持完整的应用更新生命周期：
  - 应用启动后 15 秒自动检查更新
  - 每 6 小时定期自动检查一次
  - 手动检查更新按钮，无需等待定时器
  - 实时下载进度显示和速度统计
  - 下载完成后自动提示安装
- **系统设置页**: 新增专用页面管理应用更新和系统信息：
  - 当前版本显示和运行环境标识
  - 自动更新启用/禁用开关（支持配置持久化）
  - 检查更新按钮和下载进度条
  - 发布说明和更新日志展示
  - 数据目录快速访问
- **发布历史页**: 新增页面显示所有历史版本的发布信息：
  - 从 GitHub Releases API 自动获取发布历史
  - 版本号、发布日期和完整更新说明展示
  - 支持按版本浏览历史更新内容

### 🔧 优化 (Improvements)

- **更新配置管理**: 配置文件存储在 `userData/updater-config.json`，支持用户自定义自动更新偏好
- **GitHub 集成**: 利用 GitHub Releases API 实现发布元数据和日志的自动同步，无需手动维护
