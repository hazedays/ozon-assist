<template>
  <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 mb-8 group transition-all hover:border-blue-200">
    <div class="flex items-start justify-between">
      <div class="flex gap-4">
        <div class="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center text-ozon group-hover:scale-110 transition-transform">
      ...
      <div class="flex gap-2">
        <label class="cursor-pointer px-5 py-2.5 bg-slate-800 hover:bg-slate-900 text-white rounded-xl text-xs font-bold transition-all flex items-center gap-2 shadow-sm active:scale-95">
    ...
    <div class="mt-6 border-2 border-dashed border-slate-100 rounded-xl p-8 flex flex-col items-center justify-center gap-3 transition-colors hover:bg-slate-50 cursor-pointer" @dragover.prevent="isDragging = true" @dragleave.prevent="isDragging = false" @drop.prevent="handleDrop"
      @click="fileInput?.click()" :class="{ 'border-blue-200 bg-blue-50/50': isDragging }">
      <div class="w-10 h-10 rounded-full bg-white shadow-sm flex items-center justify-center text-slate-300">
        <i class="ri-download-cloud-2-line text-xl"></i>
      </div>
      <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed text-center">
        将图片文件拖拽至此处 <br />
